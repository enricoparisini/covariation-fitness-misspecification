...
class VAE_model(nn.Module):
    ...
    def sample_latent_prior(self):
        """
        Samples a latent vector from the prior distribution on latent space
        via reparametrization trick
        """
        return torch.randn_like(torch.zeros(self.z_dim)).to(self.device)
    ...
    def annealing_factor(self, annealing_warm_up, annealing_cyclic, training_step):
        """
        Annealing schedule of KL to focus on reconstruction error in early stages of training
        """
        if not annealing_cyclic:
            if training_step < annealing_warm_up:
                return training_step/annealing_warm_up
            else:
                return 1
        else:
            step_period = training_step % (2*annealing_warm_up)
            if step_period < annealing_warm_up:
                return step_period/float(annealing_warm_up)
            else:
                return 1    
    ...
    def loss_function(self, x_recon_log, x, mu, log_var, kl_latent_scale, kl_global_params_scale, annealing_warm_up, annealing_cyclic, training_step, Neff):
        """
        ...
        warm_up_scale = self.annealing_factor(annealing_warm_up, annealing_cyclic, training_step)
    ...
    def train_model(self, data, training_parameters):
        ...
        for training_step in tqdm.tqdm(range(1,training_parameters['num_training_steps']+1), desc="Training model"):
            #print([np.shape(batch_order), shape(seq_sample_probs)])
            ...
            neg_ELBO, BCE, KLD_latent, KLD_decoder_params_normalized = self.loss_function(recon_x_log, x, mu, log_var, 
                                                                                          training_parameters['kl_latent_scale'], 
                                                                                          training_parameters['kl_global_params_scale'], 
                                                                                          training_parameters['annealing_warm_up'],
                                                                                          training_parameters['annealing_cyclic'],
                                                                                          training_step, self.Neff_training)
            ...
            if training_step % training_parameters['save_model_params_freq']==0:
                self.save(model_checkpoint=training_parameters['model_checkpoint_location']+os.sep+self.model_name+"_step_"+str(training_parameters['starting_step']+training_step),
    ...
    def test_model(self, x_val, weights_val, batch_size):
        ...
        with torch.no_grad():
            ...
            neg_ELBO, BCE, KLD_latent, KLD_global_parameters = self.loss_function(recon_x_log, x, mu, log_var, kl_latent_scale=1.0, 
                                                                                  kl_global_params_scale=1.0, annealing_warm_up=0,
                                                                                  annealing_cyclic=False, 
                                                                                  training_step=1, Neff = self.Neff_training) 
            #set annealing factor to 1
