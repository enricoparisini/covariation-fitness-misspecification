...
if __name__=='__main__':
    ...
    parser.add_argument('--load_model_location', type=str, default=None, help='Location from where a model checkpoint is loaded prior to training')   
    ...
    data = data_utils.MSA_processing(
            ...
            theta=theta,
            use_weights=True,
            weights_location=args.MSA_weights_location + os.sep + protein_name + "_" + args.model_name_suffix + '.npy'
    )
    ...
    print(f"The model is on the device {model.device}")

    #Added option to load the model from a previous checkpoint:
    if args.load_model_location is not None:
        try:
            checkpoint_name = args.load_model_location
