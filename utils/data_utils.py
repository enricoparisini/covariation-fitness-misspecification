...
class basic_MSA_processing:
    def __init__(self, 
        ...
        - file_name_label_to_seq_out: (string) the location of the output file containing the dictionary seq_name_to_sequence
        """
        np.random.seed(2021)
        ...
        self.file_name_label_to_seq_out = file_name_label_to_seq_out
        self.advanced_preprocess_MSA = advanced_preprocess_MSA
              
        self.alphabet = "ACDEFGHIKLMNPQRSTVWY"
        self.alphabetg = "-ACDEFGHIKLMNPQRSTVWY"
        self.q = len(self.alphabetg)
        self.aa_dict = {}
        self.aag_dict = {}
        for i,aa in enumerate(self.alphabet):
            self.aa_dict[aa] = i
        for i,aa in enumerate(self.alphabetg):
            self.aag_dict[aa] = i


    def msa_to_dict(self):
        self.label_to_seq = defaultdict(str)
        name = ""
        with open(self.MSA_location, "r") as msa_data:
            for i, line in enumerate(msa_data):
                ...
                if line.startswith(">"):
                    ...
                    self.label_to_seq[name] += line
 
        ## MSA pre-processing to remove inadequate columns and sequences
        if self.preprocess_MSA:
            msa_df = pd.DataFrame.from_dict(self.label_to_seq, orient='index', columns=['sequence'])
            # Data clean up
            ...
            if self.advanced_preprocess_MSA:
                ...
                # Overwrite label_to_seq with clean version
                self.label_to_seq = defaultdict(str)
                for seq_idx in range(len(msa_df['sequence'])):
                    self.label_to_seq[msa_df.index[seq_idx]] = msa_df.sequence[seq_idx]

        self.focus_seq = self.label_to_seq[self.focus_seq_name][:]
        if self.keep_cols is None:
            self.focus_cols = [ix for ix, s in enumerate(self.focus_seq) if s == s.upper() and s!='-'] 
        else:
            self.focus_cols = list(range(self.keep_cols[0], self.keep_cols[1]+1)) 

        self.focus_seq_trimmed = [self.focus_seq[ix] for ix in self.focus_cols]
        self.L = len(self.focus_cols)
        self.seq_len = len(self.focus_cols)
        self.alphabet_size = len(self.alphabet)

        # Move all letters to CAPS; keeps focus columns only
        for seq_name,sequence in self.label_to_seq.items():
            sequence = sequence.replace(".","-")
            self.label_to_seq[seq_name] = [sequence[ix].upper() for ix in self.focus_cols]
        ...
        if self.remove_sequences_with_indeterminate_AA_in_focus_cols:
            ...
            for seq_name in seq_names_to_remove: 
                del self.label_to_seq[seq_name]

        if self.file_name_label_to_seq_out is not None:
            np.save(self.file_name_label_to_seq_out, self.label_to_seq)
        
        return self.label_to_seq



class MSA_processing(basic_MSA_processing): 
    def __init__(self, 
        MSA_location="",
        theta=0.2,
        use_weights=True,
    ...
    def gen_alignment(self):
        ...
        self.one_hot_encoding = np.zeros((len(self.label_to_seq.keys()),len(self.focus_cols),len(self.alphabet)))
